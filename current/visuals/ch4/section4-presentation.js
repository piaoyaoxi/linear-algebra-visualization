(() => {
  ...
  function currentMatrix(preset, progress) {
    ...
    return multiply(interpolate(IDENTITY, inverse(preset.matrix), progress - 1), preset.matrix);
  }
  ...
  function stageCopy(preset, progress) {
    ...
    if (progress <= 1.02) {
      return {
        label: "得到输出",
        formula: "Ax",
        note: preset.singular ? "二维信息已经被压到一条直线上。" : "继续向右，开始施加逆矩阵。",
      };
    }
    if (preset.singular) {
      return {
        label: "无法唯一撤销",
        formula: "A^{-1}\\ \text{不存在}",
        note: "不同输入已经落到同一个输出，无法判断原来是哪一个。",
      };
    }
    if (progress < 1.96) {
      return { label: "正在应用 A^{-1}", formula: "A_s^{-1}Ax", note: "逆矩阵从输出端反向撤销原来的作用。" };
    }
