...
(() => {
  ...
  function mountS1(root){
    ...
    function paint(){const C=presets[state.p],B=M().symmetrize(M().congruence(A,C)),y=vectors[state.v],x=M().matVec(C,y),l=M().qForm(A,x),r=M().qForm(B,y),det=M().det2(C),ok=Math.abs(det)>1e-8;surface($(root,'[data-s1-a-canvas]'),A,'A · 原坐标',x);surface($(root,'[data-s1-b-canvas]'),B,'B · 新坐标',y);$(root,'[data-s1-c]').innerHTML=M().matrixHtml(C);$(root,'[data-s1-b]').innerHTML=M().matrixHtml(B);$(root,'[data-s1-det]').textContent=fmt(det,4);$(root,'[data-s1-x]').textContent=`(${x.map(v=>fmt(v)).join(', ')})`;$(root,'[data-s1-left]').textContent=fmt(l,4);$(root,'[data-s1-right]').textContent=fmt(r,4);const s=$(root,'[data-s1-status]');s.className=`ch5-status ${ok?'is-ok':'is-warn'}`;s.textContent=ok?'合同成立':'不是合同：换元不可逆';$(root,'[data-s1-title]').textContent=ok?'坐标换了，二次型没有换':'一个方向被压掉，无法反解';$(root,'[data-s1-copy]').textContent=ok?'det C≠0，新旧变量可互相恢复；两个红点的高度完全相同。':'代数恒等式仍成立，但 det C=0 对应退化变量代入，方向信息已经丢失，合同所需的可逆条件未满足。';}
