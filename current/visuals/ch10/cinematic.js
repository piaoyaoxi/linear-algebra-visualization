(() => {
  ...
  const arrow = (vector, origin, scale, role, label, opacity = 1) => {
    const end = mapPoint(vector, origin, scale);
    const marker = role === "amber" ? "cinema-arrow-amber" : "cinema-arrow-cyan";
    return `<g opacity="${opacity}"><line class="cinema-vector is-${role}" x1="${origin[0]}" y1="${origin[1]}" x2="${end[0]}" y2="${end[1]}" marker-end="url(#${marker})"/><circle class="cinema-vector-end is-${role}" cx="${end[0]}" cy="${end[1]}" r="5"/><text class="cinema-vector-label is-${role}" x="${end[0] + 12}" y="${end[1] - 10}">${label}</text></g>`;
  };
  ...
  function renderDual(state) {
    ...
    const basicOpacity = mode < 2 ? 1 : mode === 2 ? 0.42 : 0.16;
    const basisOpacity = mode === 2 ? 1 : 0;
    const pullOpacity = mode === 3 ? 1 : 0;
  ...
  function mount(section, root) {
    ...
    let playToken = 0;
    const paint = () => {
      svg.innerHTML = scene.render(current);
