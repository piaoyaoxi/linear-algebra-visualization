(() => {
  ...
  function sceneSvg(v1, v2, showRedundant, target, mode) {
    ...
    if (mode === "structure") {
      ...
      inner += U().softArrow([0, 0], v1, "is-u", "", mainConfig) + U().softArrow([0, 0], v2, "is-w", "", mainConfig);
      inner += U().softArrow([0, 0], first, "is-u-soft", "", mainConfig) + U().softArrow(first, target, "is-w-soft", "", mainConfig);
  ...
  function renderInteractive(root, section) {
    ...
    function render(rebind = true) {
      ...
      host.querySelector("[data-target-x]")?.addEventListener("input", (event) => {
        target[0] = Number(event.target.value);
        render();
      });
      host.querySelector("[data-target-y]")?.addEventListener("input", (event) => {
