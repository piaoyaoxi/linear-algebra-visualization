(() => {
  ...
  function sumVsUnionFigure() {
    ...
    const b = [-0.25, 1.1];
    const sum = U().add(a, b);
    const inner = U().planeGrid(config) + U().line(a, "is-u", "U", [0, 0], config) + U().line(b, "is-w", "W", [0, 0], config) + U().softArrow([0, 0], a, "is-u", "u", config) + U().softArrow(a, sum, "is-w", "w", config) + U().softArrow([0, 0], sum, "is-result", "u+w", config);
    return `<svg class="ch6-sum-union-figure" viewBox="0 0 400 230" role="img" aria-label="来自两个子空间的向量相加得到和空间中的新向量">${inner}</svg>`;
  }
  ...
  function renderFormal(root) {
    const modules = [
      U().moduleBlock("01", "交空间收集共同拥有的方向", "同时属于 U 与 W 的向量组成新的子空间", `<div class="ch6-definition-pair"><article><span>交空间</span>${U().texDisplay("U\\cap W=\\{v:v\\in U\\text{ 且 }v\\in W\\}")}<p>它继承含零、加法封闭与数乘封闭。</p></article><article><span>包含关系特例</span>${U().texDisplay("U\\subseteq W\\Rightarrow U\\cap W=U")}<p>较小的子空间就是全部公共方向。</p></article></div>`),
      U().moduleBlock("02", "和空间收集所有可以合成的方向", "它不是简单把两个集合并排放在一起", `<div class="ch6-sum-definition"><div>${sumVsUnionFigure()}</div><div>${U().texDisplay("U+W=\\{u+w:u\\in U,\\ w\\in W\\}")}<p>图中绿色向量通常既不在 U，也不在 W，却属于 U+W。</p><div class="ch6-warning-note"><strong>${U().texInline("U\\cup W")} 一般不是子空间</strong><p>分别取 u∈U 与 w∈W，u+w 往往离开集合并。</p></div></div></div>`),
      U().moduleBlock("03", "维数公式是一笔重复方向的账", "dim U 与 dim W 都计算了交空间中的方向", dimensionBookkeeping()),
      U().moduleBlock("04", "求 U+W 的基：合并生成组，再删冗余", "交空间决定需要删掉多少重复方向", `<div class="ch6-extraction-steps"><div><span>1</span><strong>写出 U 与 W 的基</strong><p>合并后一定生成 U+W。</p></div><div><span>2</span><strong>做线性无关筛选</strong><p>删去能由前面向量表示的方向。</p></div><div><span>3</span><strong>剩余向量形成和空间的基</strong><p>向量个数就是 dim(U+W)。</p></div></div>`),
      U().moduleBlock("05", "代表例题的精确结果", "不仅判断维数，还要写出交空间与和空间的基", `<div class="ch6-example-result"><div>${U().texDisplay("U=\\operatorname{span}\\{(1,0,1)^T,(0,1,1)^T\\}")}${U().texDisplay("W=\\operatorname{span}\\{(1,1,0)^T,(1,-1,2)^T\\}")}</div><div class="ch6-example-arrow">→</div><div>${U().texDisplay("U\\cap W=\\operatorname{span}\\{(1,0,1)^T\\}")}${U().texDisplay("U+W=\\mathbb{R}^3")}</div></div><p class="ch6-example-basis">和空间可取基 ${U().texInline("(1,0,1)^T,(0,1,1)^T,(1,1,0)^T")}，维数账为 ${U().texInline("2+2-1=3")}。</p>`),
    ];
    root.innerHTML = U().formalShell("交与和：公共方向与合成范围", "交空间回答“两个子空间共同拥有什么”，和空间回答“把两边方向一起使用能到达哪里”。维数公式把这两种信息联系起来。", modules, "下一节进一步要求和空间中的分解具有唯一性，这就得到直和。");
  }
  ...
  function r2Visual(same, angleU, angleW) {
    ...
    if (same) {
      ...
      inner += U().softArrow(uPart, sum, "is-w", "w");
    ...
    return U().planeSvg(inner, same ? "同一条直线的交与和" : "两条不同直线的交与和");
  }
  ...
  function renderInteractive(root, section) {
    ...
    function render() {
      ...
      host.innerHTML = U().labShell({
        ...
        focus: info.kind === "r2" ? "先看 U 与 W 是否是同一条直线，再看绿色 u+w 落在直线还是整个平面。" : "先找图中同时属于 U 和 W 的公共方向。",
