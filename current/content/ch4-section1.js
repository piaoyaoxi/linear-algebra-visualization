defineChapter4Section("matrix-language", {
  ...
  concepts: [
    { label: "行、列与阶", text: `${texInline("m\\times n")} 表示 m 行 n 列。只有行数和列数都相同的矩阵，才谈得上逐项相等。` },
    { label: "元素记号", text: `${texInline("a_{ij}")} 表示第 i 行第 j 列的元素；下标先读行，再读列。` },
    { label: "方程组背景", text: "方程组的系数可以排成矩阵，把许多同类关系压缩为一个统一对象。" },
    { label: "先看列", text: "把矩阵看成变换记录时，第 j 列给出第 j 个标准基向量经过变换后的坐标。" },
  ],
  textbook: {
  ...
  example: {
    ...
    steps: [
      ...
      "两列不共线，因此它们仍然张成整个平面；网格表现为横向拉伸加剪切，平面没有坍缩。",
    ],
  },
  quiz: [
    { question: `在 ${texInline("m\\times n")} 矩阵中，${texInline("a_{ij}")} 的两个下标分别表示什么？`, answer: "第一个下标 i 表示行号，第二个下标 j 表示列号。" },
    { question: `矩阵 ${texInline("A")} 的第 j 列在“变换记录”的视角下表示什么？`, answer: `它表示标准基向量 ${texInline("e_j")} 经 A 作用后的坐标，即 ${texInline("Ae_j")}。` },
    { question: "两列都非零的 2 阶矩阵，是否一定不会把平面压扁？", answer: "不一定。若两列共线，它们仍只能提供一个方向，网格会坍缩到一条直线。" },
  ],
  summary: [
