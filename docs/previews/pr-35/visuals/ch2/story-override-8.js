/* Final vector-language override for Chapter 2 §8. */
(() => {
  ...
  function mount(root) {
    ...
    function renderLaplace() {
      ...
      root.querySelector("[data-lap-cols]").textContent = info.cols.map((col) => col + 1).join("、");
      root.querySelector("[data-lap-comp]").textContent = info.complementCols.map((col) => col + 1).join("、");
    ...
    function geometry(matrix) {
      ...
      return {
        ...
        a1,
        a2,
        p1: mapPoint(origin, scale, a1),
        p2: mapPoint(origin, scale, a2),
        polygon: pointsString(parallelogram(origin, scale, a1, a2)),
      };
    }

    function spanLine(currentGeometry) {
      const nonzero = Math.hypot(...currentGeometry.a1) > 1e-8 ? currentGeometry.a1 : currentGeometry.a2;
      const norm = Math.hypot(...nonzero);
      if (norm < 1e-8) return "";
      const unit = [nonzero[0] / norm, nonzero[1] / norm];
      const q1 = mapPoint(currentGeometry.origin, 95, [-3.8 * unit[0], -3.8 * unit[1]]);
      const q2 = mapPoint(currentGeometry.origin, 95, [5.6 * unit[0], 5.6 * unit[1]]);
    ...
    function renderProduct(current, ghost, label) {
      ...
      const ghostGeometry = geometry(ghost);
      prodSvg.querySelector("[data-product-ghost]").setAttribute("points", ghostGeometry.polygon);
      prodSvg.querySelector("[data-product-current]").setAttribute("points", currentGeometry.polygon);
      const collapsed = Math.abs(M().det2(current)) < 1e-8;
      prodSvg.querySelector("[data-product-vectors]").innerHTML = `${collapsed ? spanLine(currentGeometry) : ""}${vectorMarkup(currentGeometry.origin, currentGeometry.p1, "primary", "v₁")}${vectorMarkup(currentGeometry.origin, currentGeometry.p2, "secondary", "v₂")}<circle class="story-origin" cx="${currentGeometry.origin[0]}" cy="${currentGeometry.origin[1]}" r="4"/>`;
    ...
    root.querySelectorAll("[data-story8-tab]").forEach((button) => button.addEventListener("click", () => {
      ...
      root.querySelector("[data-lap-message]").hidden = productTab;
      root.querySelector("[data-product-message]").hidden = !productTab;
