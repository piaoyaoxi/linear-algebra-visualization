defineChapter10Section({
  ...
  tags: ["线性函数", "线性泛函", "核空间", "等值超平面"],
  intro:
    "线性函数也称线性泛函。矩阵的一行把 " + texInline("x") + " 读成 " + texInline("a^Tx") + "；求值把多项式 " + texInline("p") + " 读成 " + texInline("p(t_0)") + "；积分把函数 " + texInline("g") + " 读成 " + texInline("\\int g") + "。它们都把一个线性空间中的对象压缩成标量，并且完整保留加法与数乘。平面上的平行等值线只是这种抽象读取方式在二维实空间里的可见模型。",
  textbook: {
  ...
  summary: [
    "线性函数（线性泛函）是从向量空间到标量域的线性映射；方程的一行、求值、迹和积分都是代表例子。",
