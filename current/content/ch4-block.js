(() => {
  ...
  window.registerAlgebraChapter = function registerChapterWithBlockSections(chapter) {
    if (chapter?.id === "ch4" && Array.isArray(chapter.sections)) {
      const section5 = chapter.sections.find((item) => item?.id === "block-matrices");
      const section7 = chapter.sections.find((item) => item?.id === "block-elementary-applications");

      if (section5) {
        Object.assign(section5, {
          number: "§5",
          textbookSection: "矩阵的分块",
          title: "矩阵的分块",
          navTitle: "矩阵的分块",
          question: "为什么把大矩阵按有意义的行列切开后，计算和结构都会更清楚？",
          goal: "理解分块不是随意画格子：块的尺寸必须匹配；块乘法仍然是行乘列；块对角结构表示彼此独立的子系统。",
          tags: ["分块", "块乘法", "块对角"],
          intro: "分块矩阵不是把一个大矩阵切碎而已。正确的切法会把问题里的子系统、输入输出分组或反复出现的结构显露出来；之后每一块就像一个“较大的元素”那样参与运算。",
          concepts: [
            { label: "分块的前提", text: "行和列必须按一致的尺寸切分；不是任何画出的格子都能参与同一种运算。" },
            { label: "块乘法", text: "输出块 Cᵢⱼ 仍然来自 A 的第 i 个块行和 B 的第 j 个块列的配对。" },
            { label: "块对角", text: "非对角块为零时，两个子系统不互相耦合，可以分别计算。" },
            { label: "结构优先", text: "分块的价值是先看见关系，再缩短计算；不是单纯让公式变长。" },
          ],
          textbook: {
          ...
          quiz: [
            {
              question: "为什么分块时不能只看图形上切得整不整齐？",
              answer: "因为块要参与加法或乘法，行列分组必须和各块的尺寸兼容；否则相应的块乘积没有定义。",
            },
            {
              question: `在 ${texInline("C=AB")} 的块乘法里，${texInline("C_{21}")} 由哪一行块和哪一列块配对？`,
              answer: "由 A 的第二块行和 B 的第一块列配对。",
            },
            {
              question: "块对角矩阵为什么可以分别处理不同块？",
              answer: "因为非对角块为零，一个块的输入不会流向另一个块的输出，子系统之间没有耦合。",
            },
          ],
          summary: [
            "分块要先保证尺寸与行列分组匹配。",
            "块乘法没有改变本质：仍然是块行乘块列。",
            "块对角结构把互不影响的子系统直接写在矩阵形状里。",
          ],
      ...
      if (section7) {
        Object.assign(section7, {
          ...
          quiz: [
            {
              question: `块行操作 ${texInline("R_2\\leftarrow R_2-CR_1")} 里，为什么 C 不能随便选尺寸？`,
              answer: "因为 CR₁ 必须和 R₂ 具有相同的列结构，才能相减；尺寸匹配是块操作合法的条件。",
            },
            {
              question: "分块初等矩阵如何构造？",
              answer: "和普通初等矩阵完全一样：对分块单位矩阵做同一个块行操作。",
            },
            {
              question: "块消元后为什么更容易解系统？",
              answer: "因为耦合块被消去后，系统变成块上三角或块对角形式，可以先解上面的块，再按块回代。",
            },
          ],
          summary: [
            "分块初等变换不是新魔法：它是普通行变换在块层面的延伸。",
            "左乘分块初等矩阵仍然改变块行。",
            "块消元的目的，是把耦合结构改写成可以按块求解的结构。",
          ],
