(() => {
  ...
  function renderFormal(root) {
    const modules = [
      ...
      U().moduleBlock("03", "几何上先看是否经过原点", "直线或平面形状相同，平移后却会失去线性结构", `<div class="ch6-origin-compare"><article><span>线性子空间</span><div class="ch6-origin-sketch is-linear"><i></i><b>0</b></div><h4>过原点</h4><p>例如 ${U().texInline("x+y+z=0")}。</p></article><article><span>仿射集合</span><div class="ch6-origin-sketch is-affine"><i></i><b>0</b></div><h4>平移离开原点</h4><p>例如 ${U().texInline("x+y+z=1")}。</p></article></div><div class="ch6-reading-note"><strong>必要但不充分</strong><p>经过原点仍需检查加法与任意数乘。第一象限经过原点，却不是子空间。</p></div>`),
    ...
    root.innerHTML = U().formalShell("线性子空间：形状之外，还要保留全部线性运算", "子空间判定的关键不是“看起来像直线或平面”，而是原空间中的任意线性组合是否仍留在这个集合里。", modules, "下一节把两个子空间放在一起，分别研究它们共享什么、合起来能生成什么。");
  }
