(() => {
  ...
  function staticCoordinateFigure() {
    ...
    return `<div class="ch6-same-vector-figure"><article><span>青色骨架：基 U</span>${left}<strong>[v]ᵤ = ${U().formatVector(vector)}</strong></article><div class="ch6-fixed-object"><span>同一个 v</span><b>端点不动</b></div><article><span>橙色骨架：基 W</span>${right}<strong>[v]ᵥ = ${U().formatVector(y)}</strong></article></div>`;
  }

  function renderFormal(root) {
    const modules = [
      U().moduleBlock("01", "同一个向量可以有不同坐标", "坐标不是向量本身，而是相对有序基的系数", `${staticCoordinateFigure()}<div class="ch6-reading-note"><strong>始终固定的对象</strong><p>两幅图中白色向量 v 的起点和终点完全相同。变化的是彩色方向骨架、网格和坐标数字。</p></div>`),
  ...
  function renderInteractive(root, section) {
    ...
    const passiveTargets = {
      shear: { label: "剪切基", matrix: [[1, 0.35], [0.55, 1]], note: "橙色方向骨架变斜，但仍保留两条独立方向。" },
      rotate: { label: "旋转基", matrix: [[0.78, -0.63], [0.63, 0.78]], note: "橙色方向骨架整体旋转，白色向量位置不动。" },
    ...
    function render(rebind = true) {
      ...
      if (mode === "passive") {
        ...
        inner += U().softArrow([0, 0], [1, 0], "is-u-soft");
        inner += U().softArrow([0, 0], [0, 1], "is-u-soft");
        inner += U().softArrow([0, 0], [matrix[0][0], matrix[1][0]], "is-w");
        inner += U().softArrow([0, 0], [matrix[0][1], matrix[1][1]], "is-w2");
        inner += U().softArrow([0, 0], vector, "is-target", "同一个 v");
        legend = `<div class="ch6-stage-legend"><span class="is-u">青色：原基 U</span><span class="is-w">橙色：新基 W</span><span class="is-target">白色：固定向量 v</span></div>`;
      } else {
        inner += U().softArrow([0, 0], vector, "is-target-soft", "输入 v");
        inner += U().softArrow([0, 0], activeVector, "is-target", "输出 Aₜv");
        legend = `<div class="ch6-stage-legend"><span class="is-muted">浅色：输入 v</span><span class="is-target">白色：输出 Aₜv</span></div>`;
      }
      ...
      const readout = mode === "passive"
        ? `<div class="ch6-mode-badge is-passive"><span>被动过程</span><strong>白色向量 v 的端点始终固定</strong></div><div class="ch6-current-story"><span>当前新基 W</span><h4>${target.label}</h4><p>${target.note}</p></div><div class="ch6-coordinate-pair"><article><span>标准基 U 下</span><strong>${U().formatVector(vector)}</strong></article><b>→</b><article><span>新基 W 下</span><strong>${passiveCoordinates ? U().formatVector(passiveCoordinates) : "不存在"}</strong></article></div><div class="ch6-formula-readout">${passiveCoordinates ? U().texDisplay("[v]_W=W^{-1}U[v]_U") : U().texDisplay("\\det W=0\\Rightarrow W^{-1}\\text{ 不存在}")}</div><div class="ch6-metric-grid">${U().metric("det W", "change-det")}${U().metric("P(W←U)", "change-p")}</div><div class="ch6-conclusion-box ${inv ? "is-ok" : "is-bad"}"><span>对象 / 表示</span><strong>${inv ? "对象不动，坐标表示改变" : "W 不再是一组基"}</strong></div>`
      ...
      host.innerHTML = U().labShell({
        ...
        focus: mode === "passive" ? "只盯住白色 v 的端点：橙色骨架变化时，它不能移动。" : "比较浅色输入 v 与白色输出 Aₜv；标准网格必须保持固定。",
        stage: `<div class="ch6-stage-shell">${U().planeSvg(inner, mode === "passive" ? "被动换基实验" : "主动线性变换实验")}${legend}</div>`,
        controls,
        readout,
