defineChapter1Section("polynomial-divisibility", {
  ...
  tags: ["带余除法", "整除", "相伴"],
  intro: `多项式带余除法写成 ${texInline("f=qg+r")}，其中 ${texInline("r=0")} 或 ${texInline("\\deg r < \\deg g")}。每一步用当前余式首项除以除式首项，保证余式次数严格下降，因此算法必然终止。`,
  concepts: [
  ...
  interactive: {
    type: "slot",
    title: "实验：除法阶梯",
    description: "单步播放多项式长除法，观察首项消去与余式次数下降。",
  ...
  example: {
    ...
    choices: [
      { correct: true, text: `商 ${texInline("x^2-x")}，余式 ${texInline("x-1")}，不整除。` },
      { text: `商 ${texInline("x^2+1")}，余式 0，整除。` },
      { text: `商 ${texInline("x^2-x")}，余式 0，整除。` },
      { text: "多项式除法不保证余式次数低于除式。" },
    ],
    steps: [
      `首项：${texInline("x^4/x^2=x^2")}，乘回后相减。`,
      `再取 ${texInline("-x")}，乘回后相减得 ${texInline("x-1")}。`,
      `余式次数 1 < 2，停止。不整除。`,
    ],
  },
  quiz: [
    { question: "整除的精确定义是什么？", answer: "存在 q 使 f=qg，即余式精确为 0。" },
    { question: "带余除法对余式有何限制？", answer: "r=0 或 deg r < deg g。" },
  ],
  summary: [
    "f=qg+r，余式次数严格低于除式（或为零）。",
    "整除 ⇔ r=0。",
    "相伴是非零常数倍。",
  ],
  exercises: ["对给定 f,g 手算带余除法。"],
});
