(() => {
  ...
  function sceneSvg(v1, v2, showRedundant, target, mode) {
    const independent = Math.abs(U().cross(v1, v2)) > 0.08;
    let inner = defs() + U().planeGrid(mainConfig);
    if (mode === "structure") {
      if (independent) inner += `<rect class="ch6-plane-field" x="10" y="10" width="620" height="360" rx="22"></rect>${lattice(v1, v2, mainConfig)}${parallelogram(v1, v2, mainConfig, "is-visible")}`;
      else inner += U().line(v1, "is-u", "当前张成一条直线", [0, 0], mainConfig);
      ...
      inner += U().softArrow([0, 0], v1, "is-u", "b₁", mainConfig) + U().softArrow([0, 0], v2, "is-w", "b₂", mainConfig);
      inner += U().softArrow([0, 0], first, "is-u-soft", `${U().formatNumber(coordinates[0])}b₁`, mainConfig) + U().softArrow(first, target, "is-w-soft", `${U().formatNumber(coordinates[1])}b₂`, mainConfig);
  ...
  function renderInteractive(root, section) {
    ...
    function render(rebind = true) {
      ...
      const readout = mode === "structure" ? `<div class="ch6-basis-verdict"><div><span>有向面积</span><strong>|det(v₁,v₂)| = ${U().formatNumber(Math.abs(det), 3)}</strong></div><div><span>张成维数</span><strong>${independent ? "2 · 整个平面" : "1 · 一条直线"}</strong></div><div><span>是否线性无关</span><strong>${independent ? "是" : "否"}</strong></div><div><span>当前向量组是否为基</span><strong>${independent && !showRedundant ? "是：够用且无冗余" : independent ? "否：v₃ 是冗余方向" : "否：缺少第二个独立方向"}</strong></div></div><div class="ch6-conclusion-box ${independent ? "is-ok" : "is-warn"}"><span>几何结论</span><strong>${independent ? "平行四边形有面积，v₂ 真正增加了一个方向" : "面积塌缩为 0，v₂ 没有增加新的方向"}</strong></div>` : `<div class="ch6-coordinate-pair"><article><span>有序基 B</span><strong>(b₁,b₂)</strong></article><b>→</b><article><span>目标向量 v</span><strong>${U().formatVector(target)}</strong></article></div><div class="ch6-coordinate-reader"><span>坐标 [v]ᵦ</span><strong>${coordinates ? U().formatVector(coordinates) : "—"}</strong><span>验证</span><strong>${coordinates ? `${U().formatNumber(coordinates[0])}b₁ + ${U().formatNumber(coordinates[1])}b₂ = v` : "先让两个基方向独立"}</strong></div><div class="ch6-conclusion-box is-ok"><span>读图方法</span><strong>先沿 b₁ 走第一段，再沿 b₂ 走第二段，终点正好落到 v</strong></div>`;
      host.innerHTML = U().labShell({ title: mode === "structure" ? "拖动 v₂：看面积怎样决定维数" : "沿两个基方向走到目标向量", lead: mode === "structure" ? "不要数箭头。拖动第二个方向，观察平行四边形面积从 0 变为非 0，张成空间也会从直线扩展为平面。" : "固定一组独立基，改变目标向量。两段彩色路径就是它的两个坐标分量。", focus: mode === "structure" ? "先看半透明平行四边形是否有面积；面积为 0 时，两个箭头只是同一方向。" : "先看黑色目标 v，再沿青色和橙色两段路径追到它的终点。", stage: `<div class="ch6-stage-shell">${sceneSvg(v1, v2, showRedundant, target, mode)}</div>`, controls, readout, tasks: U().taskBlock(section), className: `ch6-basis-lab is-${mode}` });
      if (!rebind) return;

      host.querySelectorAll("[data-basis-mode]").forEach((button) => button.addEventListener("click", () => { mode = button.dataset.basisMode; if (mode === "coordinates" && !independent) v2 = [-0.15, 1.25]; render(); }));
      host.querySelectorAll("[data-basis-preset]").forEach((button) => button.addEventListener("click", () => { const preset = button.dataset.basisPreset; if (preset === "collinear") animateV2([1.05, 0.237]); if (preset === "near") animateV2([1.05, 0.38]); if (preset === "independent") animateV2([-0.15, 1.25]); }));
      host.querySelector("[data-v2-angle]")?.addEventListener("input", (event) => { const a = Number(event.target.value); const len = U().norm(v2); v2 = [Math.cos(a) * len, Math.sin(a) * len]; render(); });
      host.querySelector("[data-v2-length]")?.addEventListener("input", (event) => { const len = Number(event.target.value); const a = Math.atan2(v2[1], v2[0]); v2 = [Math.cos(a) * len, Math.sin(a) * len]; render(); });
      host.querySelector("[data-redundant]")?.addEventListener("change", (event) => { showRedundant = event.target.checked; render(); });
      ...
      if (handle) {
        handle.addEventListener("pointerdown", (event) => {
          ...
          const move = (pointerEvent) => {
            const liveSvg = host.querySelector(".ch6-basis-scene");
            if (!liveSvg) return;
            const rect = liveSvg.getBoundingClientRect();
            const x = (pointerEvent.clientX - rect.left) * 640 / rect.width;
            const y = (pointerEvent.clientY - rect.top) * 380 / rect.height;
