(() => {
  ...
  function staticCoordinateFigure() {
    ...
    const left = `<svg class="ch6-coordinate-frame" viewBox="0 0 300 220" role="img" aria-label="标准基下的向量坐标">${localGrid(I, "is-u-grid")}${U().softArrow([0, 0], [1, 0], "is-u", "u₁", config)}${U().softArrow([0, 0], [0, 1], "is-u2", "u₂", config)}${U().softArrow([0, 0], vector, "is-target", "v", config)}</svg>`;
    const right = `<svg class="ch6-coordinate-frame" viewBox="0 0 300 220" role="img" aria-label="新基下的同一向量坐标">${localGrid(W, "is-w-grid")}${U().softArrow([0, 0], [W[0][0], W[1][0]], "is-w", "w₁", config)}${U().softArrow([0, 0], [W[0][1], W[1][1]], "is-w2", "w₂", config)}${U().softArrow([0, 0], vector, "is-target", "v", config)}</svg>`;
  ...
  function renderInteractive(root, section) {
    ...
    const passiveTargets = {
      shear: { label: "剪切基", matrix: [[1, 0.35], [0.55, 1]], note: "橙色基变斜，但仍保持两条独立方向。" },
      rotate: { label: "旋转基", matrix: [[0.78, -0.63], [0.63, 0.78]], note: "橙色基整体旋转，黑色向量位置不动。" },
    ...
    function render() {
      ...
      const matrix = interpolateMatrix(I, target.matrix, progress);
      ...
      if (mode === "passive") {
        ...
        inner += U().softArrow([0, 0], [1, 0], "is-u-soft", "u₁");
        inner += U().softArrow([0, 0], [0, 1], "is-target-soft", "u₂");
        inner += U().softArrow([0, 0], [matrix[0][0], matrix[1][0]], "is-w", "w₁");
        inner += U().softArrow([0, 0], [matrix[0][1], matrix[1][1]], "is-w2", "w₂");
      ...
      const controls = `${modeControls}${presetControls}<div class="ch6-progress-control"><label>${mode === "passive" ? "换基进度" : "变换进度"}<output>${Math.round(progress * 100)}%</output><input type="range" min="0" max="1" step="0.01" value="${progress}" data-change-progress></label><p>滑到 0 时回到标准基/恒等变换；滑到 1 时到达所选目标。</p></div><div class="ch6-coordinate-sliders"><label>向量 v 横坐标 <output>${U().formatNumber(vector[0], 1)}</output><input type="range" min="-2.2" max="2.2" step="0.1" value="${vector[0]}" data-change-vx></label><label>向量 v 纵坐标 <output>${U().formatNumber(vector[1], 1)}</output><input type="range" min="-1.8" max="1.8" step="0.1" value="${vector[1]}" data-change-vy></label></div>`;

      let readout;
      if (mode === "passive") {
        readout = `<div class="ch6-mode-badge is-passive"><span>被动过程</span><strong>黑色向量 v 的端点始终固定</strong></div><div class="ch6-current-story"><span>当前新基 W</span><h4>${target.label}</h4><p>${target.note}</p></div><div class="ch6-coordinate-pair"><article><span>标准基 U 下</span><strong>${U().formatVector(vector)}</strong></article><b>→</b><article><span>新基 W 下</span><strong>${passiveCoordinates ? U().formatVector(passiveCoordinates) : "不存在"}</strong></article></div><div class="ch6-formula-readout">${passiveCoordinates ? U().texDisplay("[v]_W=W^{-1}U[v]_U") : U().texDisplay("\\det W=0\\Rightarrow W^{-1}\\text{ 不存在}")}</div><div class="ch6-metric-grid">${U().metric("det W", "change-det")}${U().metric("P(W←U)", "change-p")}</div><div class="ch6-conclusion-box ${inv ? "is-ok" : "is-bad"}"><span>对象 / 表示</span><strong>${inv ? "对象不动，坐标表示改变" : "W 不再是一组基"}</strong></div>`;
      } else {
      ...
      host.innerHTML = U().labShell({
        ...
        focus: mode === "passive" ? "只盯住黑色 v 的端点：橙色网格变化时，它不能移动。" : "比较浅色输入 v 与黑色输出 Aₜv；标准网格必须保持固定。",
