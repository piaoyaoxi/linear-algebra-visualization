(() => {
  ...
  function mountLaplacePairing(root) {
    ...
    function contribution(cols) {
      ...
      const complement = M().determinant(M().submatrix(matrix, complementRows, complementCols));
      const exponent = rows.reduce((sum, value) => sum + value + 1, 0) + cols.reduce((sum, value) => sum + value + 1, 0);
      const sign = exponent % 2 === 0 ? 1 : -1;
      return { cols, complementCols, minor, complement, sign, term: sign * minor * complement };
    }
    ...
    function render() {
      ...
      list.innerHTML = all.map((item, index) => `<button type="button" class="ch2-pair${index === selected ? " is-active" : ""}" data-pair="${index}"><span>列 ${item.cols.map((value) => value + 1).join("")}</span><strong>${M().formatNum(item.term, 2)}</strong></button>`).join("");
  ...
  function mountProduct(root) {
    ...
    function setMeters(A, B, AB) {
      ...
      root.querySelector("[data-rule-status]").textContent = Math.abs(dAB - dA * dB) < 1e-8 ? "两边一致" : "请检查数值";
    }
      ...
      try {
        ...
        await M().animateMatrix(cB, B, {
          ...
          onUpdate(intermediate) {
            const dB = M().det2(intermediate);
            root.querySelector("[data-db]").textContent = M().formatNum(dB, 3);
            root.querySelector("[data-prod]").textContent = M().formatNum(M().det2(A) * dB, 3);
          },
        });
        // The right canvas is already in state B. This animation is genuinely B → AB.
  ...
  defineChapter2Renderer("laplace-and-product", {
      ...
      formal.innerHTML = formalShell(
        ...
        module("01", "广义 Laplace 定理", "固定 k 行，遍历全部 k 列组合。", `
          <div class="ch2-def-stack">
            ...
            <article class="ch2-def"><span class="kicker">配对</span><strong>子式 × 带符号的互补子式</strong><p>对全部列组合求和得到原行列式；k=1 时回到 §6。</p></article>
          </div>
        `) + module("02", "乘法规则", "第二阶段必须从 B 后的图形继续，而非重新从单位形开始。", `
          ...
          ${proofSteps([
            ...
            "代数入口：把 AB 的列写成 A 的列的线性组合，对各列使用多重线性。",
            "含重复列的项全部为零，只保留列指标构成排列的项。",
            "剩余求和分离为 det(A) 与 det(B) 的乘积。",
          ])}
    ...
    interactive(root) {
      ...
      root.innerHTML = `
        <h2>交互实验</h2>
        <div class="ch2-lab">
          <div class="ch2-lab-head"><h3>子式配对板</h3><p>固定前两行，4×4 中共有六个两列组合。每张卡片显示该配对对最终行列式的贡献。</p></div>
        ...
        <div class="ch2-lab ch2-lab-spaced">
          ...
          <div class="ch2-note">验证状态：<strong data-rule-status></strong></div>
