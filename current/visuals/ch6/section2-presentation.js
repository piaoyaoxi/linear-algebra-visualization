(() => {
  ...
  function renderFormal(root) {
    const modules = [
      ...
      U().moduleBlock("03", "简单性质由八条公理推出", "这些结论不是额外要求，而是从前面的规则推出来的", `<div class="ch6-property-flow"><div>${U().texDisplay("0v=(0+0)v=0v+0v")}<p>两边同时消去一个 ${U().texInline("0v")}，得到 ${U().texInline("0v=0")}。</p></div><div>${U().texDisplay("(-1)v+v=(-1+1)v=0v=0")}<p>因此 ${U().texInline("(-1)v=-v")}。</p></div><div>${U().texDisplay("a0=a(0+0)=a0+a0")}<p>两边同时消去一个 ${U().texInline("a0")}，得到 ${U().texInline("a0=0")}。</p></div></div>`),
  ...
  function renderInteractive(root, section) {
    const candidates = {
      ...
      quadrant: { label: "第一象限", kind: "quadrant", arrows: [{ from: [0, 0], to: [1.2, 0.7], className: "is-u", label: "u" }, { from: [0, 0], to: [-1.2, -0.7], className: "is-bad", label: "−u" }], gates: [true, true, false], witness: "第一象限包含零向量，并且对加法封闭；但乘以 −1 后，红色向量 −u 落到第三象限。", conclusion: "不是线性空间：负数数乘失败" },
      rgb: { label: "RGB 颜色立方体 [0,1]³", kind: "rgb", u: [0.7, 0.45, 0.3], result: [1.4, 0.9, 0.6], operationLabel: "2u", gates: [true, true, false], witness: "2u 的红色通道变为 1.4，超出允许范围 [0,1]。", conclusion: "不是 ℝ 上的线性空间" },
    ...
    function render() {
      ...
      host.innerHTML = U().labShell({ title: "用最短反例检查候选集合", lead: "不要随机试很多数。按固定顺序检查零向量、加法和任意数乘；任何一道出现反例，候选集合立即出局。", focus: "先找画面中的红色对象，再回答它违反了哪一条规则。", stage: `<div class="ch6-stage-shell">${candidateVisual(candidate)}</div>`, controls, readout, tasks: U().taskBlock(section), className: "ch6-closure-lab" });
